...
def task_add(Task_name,Time_due,Task_discr=None,groupid=None,Task_prior=0,progress=0,Task_leader=__accountname__,Task_parti=None,Time_accom=None,Time_Es=datetime.datetime.utcnow(),Has_subtask=False,maintask=None):
...
def task_update_alter(target,dict_):
	if type(target)==str:
		tar=task.document(target)
		tar.update(dict_)
	else:
...
def user_login(email,pwd):
	...
	if user.to_dict()['User_pwd']!= md5(pwd) and user.to_dict()['User_pwd']!= pwd:
...
def user_get_dict(id):
	return users.document(id).get().to_dict()
...
def group_add(name=None,message='',leader=None,party=None):
	#add a new group to the database
	tardict = {u'Name': name,u'Leader':leader,u'Message':message}
...
def group_update_alter(group,name=None,message='',leader=None,party=None):
	tardict = {u'Name': name,u'Leader':leader,u'Message':message}
	if name==None and __accountname__!='':
		tardict['Name']=users.document(__accountname__).get().to_dict()['User_name']+"'s project"
	if leader==None and __accountname__!='':
